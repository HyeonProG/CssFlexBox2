...
/* order를 지정하지 않으면 더 높은 우선 순위를 가진다. */
	.order-3 {
		order: 3;
	}
	/* flex-grow 속성은 특정 아이템이 더 많은 공간을 차지해야 할 때 사용 */
	.grow-1 {
		flex-grow: 1;
	}
	/* flex-shrink 속성은 공간이 부족할 때 얼마나 줄어들지 결정 
		특정 아이템이 줄어들면서 다른 아이템과 균형을 맞춰야 할 때 활용한다.
	*/
	.shrink-1 {
		flex-shrink: 1;
	}
	.basis-100 {
		...
		/* fles(단축 속성) - flex-grow, flex-shrink, flex-basis 를 한번에 설정하는 속성 */
	.flex-combined {
		flex: 1 1 300px;
	}
</style>
